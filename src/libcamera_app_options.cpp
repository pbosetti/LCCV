...
void Options::Print() const {
  ...
  std::cerr << "    photo resolution: " << photo_width << " x " << photo_height
            << std::endl;
  std::cerr << "    video resolution: " << video_width << " x " << video_height
            << std::endl;
  ...
  if (roi_width == 0 || roi_height == 0)
    ...
    std::cerr << "    roi: " << roi_x << "," << roi_y << "," << roi_width << ","
              << roi_height << std::endl;
  if (shutter)
  ...
  if (awb_gain_r && awb_gain_b)
    std::cerr << "    awb gains: red " << awb_gain_r << " blue " << awb_gain_b
              << std::endl;
