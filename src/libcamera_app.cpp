...
std::string const &LibcameraApp::CameraId() const { return camera_->id(); }
...
void LibcameraApp::OpenCamera() {
  ...
  std::string const &cam_id =
      camera_manager_->cameras()[options_->camera]->id();
...
void LibcameraApp::ConfigureStill(unsigned int flags) {
  ...
  // Always request a raw stream as this forces the full resolution capture
  // mode. (options_->mode can override the choice of camera mode, however.)
...
void LibcameraApp::Teardown() {
  if (options_->verbose && !options_->help)
    std::cerr << "Tearing down requests, buffers and configuration"
              << std::endl;
...
void LibcameraApp::StartCamera() {
  ...
  // Build a list of initial controls that we must set in the camera before
  // starting it. We don't overwrite anything the application may have set
  // before calling us.
  if (!controls_.get(controls::ScalerCrop) && options_->roi_width != 0 &&
      options_->roi_height != 0) {
    Rectangle sensor_area =
        *camera_->properties().get(properties::ScalerCropMaximum);
  ...
  // Framerate is a bit weird. If it was set programmatically, we go with that,
  // but otherwise it applies only to preview/video modes. For stills capture we
  // set it as long as possible so that we get whatever the exposure profile
  // wants.
  ...
  if (!controls_.get(controls::ColourGains) && options_->awb_gain_r &&
      options_->awb_gain_b)
...
void LibcameraApp::StopCamera() {
  {
    // We don't want QueueRequest to run asynchronously while we stop the
    // camera.
  ...
  // An application might be holding a CompletedRequest, so queueRequest will
  // get called to delete it later, but we need to know not to try and re-queue
  // it.
...
LibcameraApp::Msg LibcameraApp::Wait() { return msg_queue_.Wait(); }
...
void LibcameraApp::queueRequest(CompletedRequest *completed_request) {
  ...
  // An application could be holding a CompletedRequest while it stops and
  // re-starts the camera, after which we don't want to queue another request
  // now.
  {
  ...
  for (auto const &p : buffers) {
    if (request->addBuffer(p.first, p.second) < 0)
      throw std::runtime_error(
          "failed to add buffer to request in QueueRequest");
  }
...
libcamera::Stream *LibcameraApp::ViewfinderStream(unsigned int *w,
                                                  unsigned int *h,
                                                  unsigned int *stride) const {
  return GetStream("viewfinder", w, h, stride);
}

libcamera::Stream *LibcameraApp::StillStream(unsigned int *w, unsigned int *h,
                                             unsigned int *stride) const {
  return GetStream("still", w, h, stride);
}

libcamera::Stream *LibcameraApp::RawStream(unsigned int *w, unsigned int *h,
                                           unsigned int *stride) const {
  return GetStream("raw", w, h, stride);
}

libcamera::Stream *LibcameraApp::VideoStream(unsigned int *w, unsigned int *h,
                                             unsigned int *stride) const {
  return GetStream("video", w, h, stride);
}

libcamera::Stream *LibcameraApp::LoresStream(unsigned int *w, unsigned int *h,
                                             unsigned int *stride) const {
...
LibcameraApp::Mmap(FrameBuffer *buffer) const {
...
void LibcameraApp::StreamDimensions(Stream const *stream, unsigned int *w,
                                    unsigned int *h,
                                    unsigned int *stride) const {
...
void LibcameraApp::setupCapture() {
  ...
  // Next allocate all the buffers we need, mmap them and store them on a free
  // list.
...
void LibcameraApp::requestComplete(Request *request) {
  ...
  CompletedRequestPtr payload(
      r, [this](CompletedRequest *cr) { this->queueRequest(cr); });
  {
    std::lock_guard<std::mutex> lock(completed_requests_mutex_);
